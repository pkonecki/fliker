...
elseif(isset($_POST['action']) && $_POST['action'] == 'modif_sup'){
	...
	if (isset($_POST['action']) && $_POST['action'] === 'new_sup')
		addSup("activite",$_POST['id_act'],$_POST['type'],$_POST['valeur'],$_POST['id_asso_adh'],$_POST['id_asso_paie'],$_POST['facultatif'],$_POST['reduction'],$promo);
	if (isset($_POST['action']) && $_POST['action'] === 'copy_old_sups')
	{
		$sups = getSup("activite",$_GET['act'],$_POST['old_promo']);
		foreach ($sups as $key => $value)
			addSup("activite",$_GET['act'],$value['type'],$value['valeur'],$value['id_asso_adh'],$value['id_asso_paie'],$value['facultatif'],$value['reduction'],$promo);
	}
		...
		else
		{
			...
			foreach ($sups as $id => $sup)
			{
				...
				if ($promo==$current_promo)
				{
					...
					print '<td><FORM action="index.php?page=5&sup='.$id.'&act='.$_GET['act'].'" method="POST">
