...
function getCoutsCreneaux(){
//Pour chaque asso, somme des supplément suivant le statut de l'adhérent
$q1="SELECT  SUM(sup.valeur) valeur_sup, sup.id_statut id_statut_sup, A.id id_asso, A.nom nom_asso
...
$q2="SELECT SUM(sup.valeur) somme_sup, sup.id_asso_adh id_asso_adh, S.id id_sec, S.nom nom_sec, AC.id id_act, AC.nom nom_act, CR.id id_cre, CR.jour jour_cre, CR.debut debut_cre, CR.fin fin_cre, CR.lieu lieu
...
$query="SELECT (S1.valeur_sup+S2.somme_sup) total,  S1.id_statut_sup, HS.id_asso, S1.nom_asso, S2.id_cre FROM
...
while($row = mysql_fetch_array($results)){
	$tab[$row['id_statut_sup']][$row['id_cre']][$row['id_asso']]['total']=$row['total'];
	$tab[$row['id_statut_sup']][$row['id_cre']][$row['id_asso']]['nom_asso']=$row['nom_asso'];
}
include("closedb.php");
return $tab;
